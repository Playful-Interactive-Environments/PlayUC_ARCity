...
public class LocalManager : MonoBehaviour
{
	...
	public void UpdateProgress()
	{
		if(SaveStateManager.Instance != null && CurrentRank < 20)
		{
			...
			if (_currentInfluence >= _currentGoal && NetworkCommunicator !=null && !levelUnlocked)
			{
				NetworkCommunicator.UpdateData(RoleType, "Rank", 1);
	...
	public void CreateLevelTemplate()
	{
		...
		for (int i = 1; i <= _csvLeveling.rowList.Count-1; i++)
		{
			...
			level.transform.SetParent(GridGroup.transform);
